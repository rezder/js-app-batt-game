...
export class BattSvg extends Component {
    ...
    /*clickHandler2(p) {
       ...
       if (clickedPointer.type === dPos.BPCone) {
       let clickedConeix = clickedPointer.ix
       state = clickCone(this.state, clickedConeix, mv.mover(moves));
       } else {
       let handCardPointer = this.state.handCardPointer
       let scoutReturnPointer=this.state.scoutReturnPointer
       if (!handCardPointer) {
       if (dCard.isBack(clickedPointer.ix)) {//TODO should be possible even with handCardPointer split this function after clicked pos cone,scoutReturn,deck,hand and else(dish and flag)
       moveix = mv.findMovePosOnFirst(clickedPointer.ix, dPos.card
       .Players[mv.mover(moves)].Hand, moves);
       } else if ((scoutReturnPointer) && scoutReturnPointer.isEqual(clickedPointer)) {
       let res = clickScoutReturn(this.state, moves,
       clickedPointer)
       state = res.state;
       } else {
       state = {
       ...
       if (clickedPointer.pos === dPos.card.Players[0].Hand ||
       clickedPointer.pos === dPos.card.Players[1].Hand) {
       state = clickHand(this.state, handCardPointer.ix,
       clickedPointer)
       } else {
       ...
       let res = clickFlagDishDeck(this.state, moves,
       clickedPointer)
       moveix = res.moveix;
       state = res.state;
       }
